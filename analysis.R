...
gender_check <- mBT[gender=='Male' | gender == 'Female',] # discard one 'other' participants just for gender test
  
ggplot(gender_check[, .(BT = mean(BT),
               se = sd(BT) / sqrt(.N)), 
           by = gender], aes(x = gender, y = BT, ymin = BT - se, ymax = BT + se)) +
  geom_pointrange(size = 1) +
  labs(x = "participants gender", y = "average BT (ms)", tag = "a") +
  theme(text = element_text(size=20))

t.test(BT ~ gender, gender_check)
...
ggplot(stimuli[, .(mean_BT = mean(mean_BT),
  ...
  geom_pointrange(size = 1) +
  labs(x = "stimuli gender", y = "average BT (ms)", tag = "b") +
  theme(text = element_text(size=20))
...
sd.m <- sd(stimuli[stim_gender=='m']$mean_BT)
sd.m
sd.f <- sd(stimuli[stim_gender=='f']$mean_BT)
sd.f
...
### check how face scores according to each group explain BTs in the others
cor.test(mXm_dim_sc, fXm_mZ[,fXm_mean_Z])
ggplot(fXm_mZ, aes(x = fXm_mean_Z, y = mXm_dim_sc)) +
  geom_point() +
  geom_smooth(method='lm')
...
### how do odd trials BTs correlate with even trials BTs (between participants) 
...
### divide participants to 2 groups and check priority dimension correlation (only female faces for now) doesnt work for now!
# doesnt work for now!!
between_cor_list <- numeric()
for (i in 1:1000) {
  ...
  group0Xf_dim <- extractDimension(stimuli[,group0Xf_mean_Z], female_params)
  group1Xf_dim <- extractDimension(stimuli[,group1Xf_mean_Z], female_params)
  
  between_cor_list <- c(between_cor_list, cor.test(group0Xf_dim, group1Xf_dim)$estimate)
}
hist(between_cor_list)
...
# swap face params for checking face correlation with BTs with scrabbled face paramteres ----
...
# take 80% of trials from each participant, make priority dim, and see how it predicts the other 20% dim and scores
